...
def createTask(name, project):
    ...
    newTask = Task.create(name=name, project=project, order=taskOrder)
    newTask.save()
...
def countProjectTasks(project):
    tasks = Task.select().where(Task.project == project.id).count()
...
def getManyTask(projectID, idList):
    tasks = Task.select().where(Task.project == projectID, Task.order << idList)

    return tasks


def getTaskByOrder(projectID, order):
    tasks = Task.select().where(Task.project == projectID, Task.order == order)
...
def getTasksByProject(id):
    return Task.select().where(Task.project == id).order_by(Task.order)


def getCompletionForProject(projectID):
    total_tasks = Task.select().where(Task.project == projectID).count()
    ...
    complete_tasks = (
        Task.select()
        .where((Task.status == Status.DONE.value) & (Task.project == projectID))
        .count()
    )
...
def findAllTaskStatus(status, orderBy=Task.order, random=False, desc=False):
    ...
    return Task.select().where(Task.status == status).order_by(order)


def findAllTaskStatusForProject(
    status, projectID, orderBy=Task.order, random=False, desc=False
):
    ...
    return (
        Task.select()
        .where((Task.status == status) & (Task.project == projectID))
        .order_by(order)
    )
...
def findActiveTaskForProject(projectID):
    return Task.select().where(
        (Task.status == Status.IN_PROGRESS) & (Task.project == projectID)
    )
...
def updateOrder(projectID, current, destination):
    ...
    if initial < final:
        query = Task.update(order=Task.order - 1).where(
            (Task.project == projectID) & (Task.order > initial) & (Task.order <= final)
        )
        query.execute()
    elif initial > final:
        query = Task.update(order=Task.order + 1).where(
